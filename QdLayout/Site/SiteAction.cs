...
namespace QdLayout
{
    public class SiteAction : ComponentInstance2dAction
    {
        ...
        public SiteAction(IDocumentEditor docEditor) : base(docEditor)
        {
            commandCtrl.WriteInfo("命令：Site");
        }
        static SiteAction()
        {
        ...
        Step0:
            var result0 = await elementInputerInGroup.Execute("请选择已有闭合线段创建场地:");
        ...
        {
            var doc = docRt.Document;
            var siteDef = docRt.GetUseComDef($"{NamespaceKey}.建构筑物", "场地", null) as QdSiteDef;
            var site = new QdSite(siteDef);
            site.Initilize(doc);
            ...
            site.Bottom = -10;
            site.Thickness = 9000;
            site.Material = MaterialManager.GetMaterial(MaterialManager.SiteUuid);
        ...
        {
            var site = element as QdSite;
            var pen = GetDrawPen(site);
            DrawSite(canvas, site, matrix, pen);
        }
        ...
        public void DrawSite(LcCanvas2d canvas, QdSite site, Matrix3 matrix, LcPaint pen)
        {
        ...
        public override ControlGrip[] GetControlGrips(LcElement element)
        {
            ...
            var gripCenter = new ControlGrip
            {
                Element = site,
                Name = "Center",
                Position = site.BoundingBox.Center
            };
        ...
        {
            var qdSite = element as QdSite;
            var sscur = SnapSettings.Current;
            var result = new SnapPointResult { Element = element };
            if (sscur.ObjectOn && qdSite.Outline.Curve2ds.Count > 0)
            {
                foreach (var curve in qdSite.Outline.Curve2ds)
                {
        ...
        public override List<PropertyObserver> GetPropertyObservers()
        {
             ...
             new PropertyObserver()
            {
                ...
                Getter = (ele) => (ele as QdSite).Properties.GetValue<double>("Bottom"),
                Setter = (ele, value) =>
                {
                    var site = (ele as QdSite);
             ...
             new PropertyObserver()
            {
                ...
                CategoryDisplayName = "几何图形",
                PropType=PropertyType.Double,
                Getter = (ele) => (ele as QdSite).Properties.GetValue<double>("Thickness"),
                Setter = (ele, value) =>
                {
                    var site = (ele as QdSite);
                    if (!double.TryParse(value.ToString(),out var thickness)&&thickness>9000)
                        return;
                    site.Thickness= thickness;
                }
            },
            };
        }
        ...
        {
            var layer = docRt.Document.Layers.FirstOrDefault(n => n.Name == "Layout_Site");
            if (layer == null)
            {
                layer = docRt.Document.CreateObject<LcLayer>();
                layer.Name = "Layout_Site";
